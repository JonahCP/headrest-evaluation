...
ERPfile = 'StimuliVerificationTrials/11_17_23_364DEEGtests/JasonTestErp.gdf'
ERPfile1 = 'StimuliVerificationTrials/11_17_23_364DEEGtests/JasonErp1.gdf'
ERPfile2 = 'StimuliVerificationTrials/11_17_23_364DEEGtests/JasonErp2.gdf'
ERPfile3 = 'StimuliVerificationTrials/11_17_23_364DEEGtests/JasonErp3.gdf'
ERPfile4 = 'StimuliVerificationTrials/11_17_23_364DEEGtests/JasonErp4.gdf'

# Define array of files
ERPfiles = [ERPfile1, ERPfile2]
...
for file in ERPfiles:
    ...
    for eid in event_ids:
        ...
        if eid == 2:
            base_epochs = mne.Epochs(raw, events, eid, tmin=-0.25, tmax=0.8, baseline=(-0.25, -0.05), detrend=1, preload=True).copy()
            base_epochs.drop_bad(reject=dict(eog=200))
            all_epochs.insert(0, base_epochs)
        else:
            target_epochs = mne.Epochs(raw, events, eid, tmin=-0.25, tmax=0.8, baseline=(-0.25, -0.05), detrend=1, preload=True).copy()
...
fig.legend(['JZ Grand Average Base Stimuli', 'JZ Grand Average Target Stimuli'], loc='right', bbox_to_anchor=(0.5, 0.98), fancybox=True, shadow=True, ncol=2)
